...
class DataExtend
{
     ...
     * @return array
     */
    public static function arr2tree($list, $key = 'id', $pkey = 'parent_id', $skey = 'sub')
    {
        list($tree, $map) = [[], []];
        ...
        foreach ($list as $item) {
            $item = (array)$item;
            if (isset($item[$pkey]) && isset($map[$item[$pkey]])) {
    ...
    public static function arr2table(array $list, $key = 'id', $pkey = 'pid', $path = 'path', $ppath = '')
    {
        ...
        foreach (self::arr2tree($list, $key, $pkey) as $attr) {
            ...
            $tree[] = $attr;
            if (!empty($sub)) {
                $tree = array_merge($tree, self::arr2table($sub, $key, $pkey, $path, $attr[$path]));
            }
        }
        return $tree;
    }
